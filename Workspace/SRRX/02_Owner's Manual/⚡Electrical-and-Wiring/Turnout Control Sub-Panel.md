...
Its primary function of the panel is to translate low-voltage digital I2C logic commands into the bi-directional 9.8 VDC polarity shifts required to drive and stall slow-motion turnout motors in either the "Normal" or "Reverse" position.
[[Is This Better Than Commercially Available Decoders]]?
This is a photo of the ***first*** final panel. The exposed wires on the board are portected by a plexiglas cover on 2" the standoffs (hardly visible, but there). 
...
- **5× DRV8833 Dual H-Bridge Motor Driver Boards:**
    ...
    - **Operation:** Receives 3.3V logic signals from the MCP23017 pins (`IN1`/`IN2`, `IN3`/`IN4`) and routes 9.8V DC motor current to the output pins with reversible polarity (`OUT1`/`OUT2`, `OUT3`/`OUT4`).
...
- **1× Infeed Power / Bus Terminal Block:**
    - **Function:** Serves as the primary power and signal entry point for external 9.8V DC power and system common grounding.
...
1. **Logic Domain (3.2V DC):**
 ...
 2. **Motor Domain (9.8V DC):**
    - Supplied by an external 9.8V DC layout accessory power supply landing on the infeed terminal block.
	    - This is supplied by a separate AC adapter, though could be the 12V accessory bus with a step down buck board set to 9.8V. The DRV8833 boards act unreliably with more than 10.8V.
    - Carries the continuous stall current for all 10 connected switch machines.
 3. **Common Ground:**
    - The external 9.8V power supply negative ($-$), the DRV8833 ground bus, and the CSB1/MCP logic ground line are tied together at the infeed block to establish a stable zero-volt reference across the logic-to-driver interface.
### Wiring Rundown
- **9.8V Power Spine:**
	- Power coming from the terminal block is routed to a 2x12 distribution module (A and B). 24 AWG wire comes out of these ports across the plywood floor to the DRV8833.
